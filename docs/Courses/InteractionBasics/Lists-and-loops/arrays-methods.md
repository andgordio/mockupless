...
Array is a powerful data type that comes with a number of built-in **methods**. Methods are commands you can give to arrays to change their contents. 
...
Now clicking the button adds the value of the `newTask` to the array first, then clears it by assigning an empty string.
...
Next time you press the Delete button, the D is deleted because it has index 2 now. Next click will remove E and so on. This will continue until there are only two items in the array and there’s nothing to delete at index 2.
...
- Track indexes in `v-for` attribute with a temporary name.
- Add Delete button to the template-container.
...
Design a prototype that allows to move items between 2 lists. This task is a little abstract but it’s good for honing your basic list management skills:
...
1. Create an array to store notes. Populate it with a couple of notes by default, so you don’t start with an empty prototype.
