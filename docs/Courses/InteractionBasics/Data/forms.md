...
As mentioned previously, data is commonly used to save information users enter. Vue.js provides a special attribute `v-model` that allows to connect variables with basic form controls: text inputs, checkboxes, radio buttons and dropdowns. 
...
If you want to save user’s text input you need to place `v-model` attribute inside `input` or `placeholder` tags and specify the name of the variables that should store the input:
...
Everything a user enters into these controls will be stored in variables `title` and `description`. Note that placeholder is a *stylistic* attribute, which means that its value is not connected to a variable.
...
4. Put 3 text input fields in the left column and connect them to the variables.
5. In the right column, [render](./display.md#text) the values of `name` and `email` as texts. Use the value of `photo` to [render an image](./display.md#images).
...
2. Create 5 varibles, one for each control in the form
3. Create a two-column layout.
