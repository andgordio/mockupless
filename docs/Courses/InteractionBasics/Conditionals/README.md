...
Let's say you are creating a prototype with 3 views—Newsfeed, Explore and Profile—and you want to allow users to switch between tabs with controls on the right:
...
- Top navigation with app name and tabs
...
3. Add `v-if` directive to views’ content containers. This shows and hides containers based on the value of the variable.
...
You will still see all three content containers in your layout, but now you can use `currentTab` as a title by [rendering](./../Data/display.html#text) its value in the header:
...
So initially you specified the default value of `currentTab` to be `'Newsfeed'`. As a result the first content container is shown by default because its condition inside `v-if` is met. Then you added event listeners that change the value of the variable, so when users click on Explore tab for example, the value of `currentTab` is changed to `'Explore'`. When this happens the first container is hidden because its condition is not met any more, and the second container is shown since its condition is satisfied now:
...
Create a simple color palette with an ability to select a color and see its Hex code:
...
5. Apply `v-if` to the colored containers so they each of them is rendered only if a particular value is assigned to the variable. For example, the first one should be rendered only when `selectedColor` is `'red'`, the second one when `selectedColor` is whatever the name you assign on click of the second button.

If you have problems with steps 1–4, fork the [starting pen](https://codepen.io/andgordy/pen/PgzQdx?editors=1000). It inludes the layout, the variable and the event listeners. Complete step 4 and try adding more color options to the palette.
...
3. Follow the steps descirbed in the article: [create a variable](./#a-variable), [add event listeners](./#event-listeners) and finally [add conditions to content containers](./#conditions).